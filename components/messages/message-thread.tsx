...
export function MessageThread({ conversationId, conversationName }: MessageThreadProps) {
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="min-h-0 flex-1 space-y-2.5 overflow-y-auto px-3 py-3 sm:space-y-3 sm:px-5 sm:py-4">
        ...
        {messages?.map((msg) => (
          <div key={msg.id} className={cn("flex", msg.isOwn ? "justify-end" : "justify-start")}>
            <div
              className={cn(
                "max-w-[85%] rounded-2xl px-3 py-2 text-sm sm:max-w-[75%] sm:px-3.5",
                msg.isOwn
      ...
      <form
        ...
        className="flex items-center gap-2 border-t border-surface-border/60 p-2.5 sm:p-3"
      >
        <input
          type="text"
          value={draft}
          ...
          className="h-11 min-w-0 flex-1 rounded-full border border-surface-border/60 bg-canvas/50 px-4 text-sm text-ink placeholder:text-ink-faint focus:outline-none focus:ring-2 focus:ring-brand sm:h-10"
        />
        <button
          type="submit"
          disabled={!draft.trim()}
          className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-brand text-white shadow-glow transition-opacity hover:bg-brand-hover disabled:opacity-40 disabled:shadow-none sm:h-10 sm:w-10"
        >
